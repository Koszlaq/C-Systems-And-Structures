...
static FILE *plikLog;
static char nazwaPliku[64];
static pthread_mutex_t blokadaZapisu;
static atomic_int koniecWatkow = 0;
static volatile sig_atomic_t pending_poziom = -1;
static pthread_t watekLogowania, watekZmianyPoziomu, watekDump;
static atomic_int czyLogowanieAktywne = 0, aktualnyPoziomLogowania = 0, czyLoggerZainicjalizowany = 0;
static sem_t semLogowanie, semZmianaPoziomu, semDump;
...
short logger_inicjalizacja(const char *nazwa) {
    ...
    atomic_store(&aktualnyPoziomLogowania, MAX);
    atomic_store(&czyLogowanieAktywne, 1);
